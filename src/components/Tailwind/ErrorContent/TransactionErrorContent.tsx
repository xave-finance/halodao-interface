...
const ErrorContent = ({ errorObject, singleErrorMessage, closeError }: ErrorProps) => {
  const { getErrorMessage, message } = useErrorMessage()
  errorObject && getErrorMessage(errorObject)
  return (
    <div className="p-4">
      ...
      {singleErrorMessage ? (
        <div className="text-center">
          ...
          <p>{singleErrorMessage}</p>
        </div>
      ) : (
        <div className="text-center">
          <p className="font-semibold">Please show this error message {errorObject?.message} to the team on Discord.</p>
        </div>
      )}
