...
import { Currency, CurrencyAmount, ETHER, JSBI, Token, TokenAmount } from '@halodao/sdk-poc'
...
/**
 * Returns a map of token addresses to their eventually consistent token totalSupply
 */
export function useTokenTotalSuppliesWithLoadingIndicator(
  tokens?: (Token | undefined)[]
): [{ [tokenAddress: string]: TokenAmount | undefined }, boolean] {
  const validatedTokens: Token[] = useMemo(
    () => tokens?.filter((t?: Token): t is Token => isAddress(t?.address) !== false) ?? [],
