...
const PoolCardRight = ({ pool }: PoolCardRightProps) => {
  ...
  const token0Price = bigNumberToNumber(pool.tokens[0].weight.mul(1e8).div(pool.tokens[0].rate))
  const token1Price = bigNumberToNumber(pool.tokens[1].weight.mul(1e8).div(pool.tokens[1].rate))
